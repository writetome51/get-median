import { getMedianFromUnordered } from '../getMedianFromUnordered';


// Test 1: error must be triggered if no argument is passed:
let errorTriggered = false;
try {
	let result = getMedianFromUnordered();
}
catch (e) {
	errorTriggered = true;
}
...
try {
	let result = getMedianFromUnordered([1, 2, 3, 'e', 4, 5, 8]);
}
catch (e) {
	++errorsTriggered;
}
try {
	let result = getMedianFromUnordered([1, 2, 3, false, 4, 5, 8]);
}
catch (e) {
	++errorsTriggered;
}
try {
	let result = getMedianFromUnordered([1, 2, 3, 4, [], 7, 8, 9]);
}
catch (e) {
	++errorsTriggered;
}
try {
	let result = getMedianFromUnordered([1, 2, 3, 3.5, undefined, 4, 5, 8]);
}
catch (e) {
	++errorsTriggered;
}
try {
	let result = getMedianFromUnordered([1, 2, {}, 7, 8]);
}
catch (e) {
	++errorsTriggered;
}
...
let result = getMedianFromUnordered([19, 3, 5, 9, 11.2, 15, 1, 7, 17, 21, 13]);
if (result === 11.2) console.log('test 4 passed');
else console.log('test 4 failed');


// Test 5: get median accurately when array length is even:
result = getMedianFromUnordered([19, 3, 5, 7, 9, 11, 13.3, 15, 17, 21, 23, 1]);
...
result = getMedianFromUnordered(numbers);
let median = 500000;
...
result = getMedianFromUnordered([18, 3, 8, 48, 13, 23, 33, 68, 43, 53, 58, 38, 63, 28]);
median = 35.5;
